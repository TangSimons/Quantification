...
class DualThrustGM(bt.Strategy):
    ...
    def next(self):
        ...
        high_yesterday = self.datahigh[-1]
        low_yesterday = self.datalow[-1]
        close_yesterday = self.dataclose[-1]
        current_open = self.dataopen[0]
        stop_long_price = current_open+2
        stop_short_price = current_open+5
        has_long_position = self.position.size > 0
        has_short_position = self.position.size < 0

        if has_long_position:
            if stop_long_price < close_yesterday :
                self.log(f'平多仓, 开盘价 {current_open:.2f} < 昨日收盘价 {close_yesterday:.2f}')
                self.count = 0
                self.order = self.close()

        elif has_short_position:
            if stop_short_price > close_yesterday :
                self.log(f'平空仓, 开盘价 {current_open:.2f} > 昨日收盘价 {close_yesterday:.2f}')
                self.count = 0
                self.order = self.close()

        if self.position.size == 0 and self.count ==0:
            if current_open > high_yesterday and self.count ==0:
                self.log(f'开多仓, 开盘价 {current_open:.2f} > 前高 {high_yesterday:.2f}')
                self.count = 1
                self.order = self.buy()


            elif current_open < low_yesterday and self.count ==0:
                self.log(f'开空仓, 开盘价 {current_open:.2f} < 前低 {low_yesterday:.2f}')
                self.count = 1
...
if __name__ == '__main__':
    ...
    # 加载策略
    cerebro.addstrategy(DualThrustGM)
